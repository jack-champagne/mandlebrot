...
def color_clip(num):
    if num < 0: return 0
    return num if num < 255 else 255
...
class Mandlebrot:
    ...
    def draw_coords(self):
        lx1, lx2 = self.coords_to_pixel(-2, 0)
        lx3, lx4 = self.coords_to_pixel(1, 0)
        self.graph_area.create_line(lx1, lx2, lx3, lx4,  fill="#ffffff")

        ly1, ly2 = self.coords_to_pixel(0, 1)
        ly3, ly4 = self.coords_to_pixel(0, -1)
...
# -        return True
        ...
        r, g, b = (int(color_clip(50 * (ax * ax + ay * ay))), int(color_clip(50 * (ax * ax + ay * ay))), int(color_clip(50 * (ax * ax + ay * ay))))

        col = color_to_hex(r, g, b)
